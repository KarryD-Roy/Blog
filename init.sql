-- =====================================================
-- Blog 数据库初始化脚本
-- 由 MySQL Docker 容器的 docker-entrypoint-initdb.d/ 自动执行
-- 仅在首次初始化时执行一次
-- =====================================================

-- 创建数据库（如果 docker-compose 中已指定 MYSQL_DATABASE 则可省略）
...
-- ========== 初始数据 ==========
-- 注：用户和角色由应用启动时 DataInitConfig 通过 BCryptPasswordEncoder 统一创建，
--     确保密码哈希与 SecurityConfig 中的 BCryptPasswordEncoder(10) 完全一致。
--     临时禁用外键检查以允许种子数据在用户创建前插入（DataInitConfig 会自动创建 id=1 的 admin 用户）。
...
(1, '后端开发', 'Java、Spring 生态和服务端技术'),
(2, 'AI 与大模型', '大语言模型、智能体、RAG 等'),
(3, '数据库与中间件', '数据库、Redis、消息队列等技术分享'),
(4, '系统与运维', 'Linux、Docker、运维与部署'),
(5, '前端开发', 'Vue、前端工程化与交互'),
(6, '架构与软技能', '设计模式、架构思路与团队协作');

INSERT INTO posts (id, title, summary, content, user_id, category_id, tags, view_count, pinned, status, created_at, updated_at) VALUES
(1, '基于 Spring Boot 和 MyBatis-Plus 搭建个人博客后端', '从零搭建一个支持分页、分类的博客后端服务，实现常见 CRUD 能力。', '本文将介绍如何使用 Spring Boot 3.x + MyBatis-Plus 搭建一个简洁易扩展的博客后端，包括实体设计、分页查询、统一返回结果以及基础的异常处理思路。', 1, 1, 'Spring Boot,MyBatis-Plus,后端实战', 12, 1, 'PUBLISHED', NOW(), NOW()),
(2, '使用 Redis 为博客系统加速：缓存常见实践', '讲解如何在个人博客中使用 Redis 做缓存，提高首页和文章详情的响应速度。', '在真实场景中，博客首页文章列表和热门技能信息访问频率很高，我们可以借助 Redis 将这些数据缓存起来，并设计合理的失效策略以避免缓存雪崩。', 1, 3, 'Redis,缓存,性能优化', 8, 0, 'PUBLISHED', NOW(), NOW()),
(3, '初探 LangChain 与 Spring AI：让博客具备问答能力', '将 AI 能力集成到博客中，让读者可以就文章内容进行自然语言问答。', '本文会演示如何使用 LangChain 与 Spring AI 封装大模型调用，将博客文章内容索引到向量库中，实现一个简单的检索增强生成（RAG）问答功能的雏形设计思路。', 1, 2, 'LangChain,Spring AI,RAG', 5, 0, 'PUBLISHED', NOW(), NOW()),
(4, '使用 Docker 一键启动博客技术栈', '通过 Docker Compose 将 MySQL、Redis、Nginx 等组件整合在一起，提升环境搭建效率。', '对于经常需要在不同机器上部署博客的开发者，通过 Docker 化整个技术栈可以极大缩短环境准备时间，并保证环境一致性。', 1, 4, 'Docker,Docker Compose,运维', 3, 0, 'PUBLISHED', NOW(), NOW()),
(5, 'MySQL 索引原理与慢查询优化实战', '深入理解 B+ 树索引，并结合实际慢查询案例做优化。', '从索引数据结构、最左前缀、覆盖索引到执行计划解读，帮助你在生产环境中快速定位并优化慢 SQL。', 1, 3, 'MySQL,索引,性能优化', 15, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY)),
(6, 'Spring Cloud Gateway 网关路由与过滤器', '使用 Gateway 统一入口，实现路由、限流与鉴权。', '介绍 Spring Cloud Gateway 基本配置、Predicate、Filter 以及如何与 Nacos 集成做动态路由。', 1, 1, 'Spring Cloud,Gateway,微服务', 7, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 2 DAY)),
(7, 'RAG 检索增强生成：从原理到落地', '梳理 RAG 流程中的分块、向量化与检索策略。', '讲解文档分块、Embedding 模型选型、向量库检索以及如何与 LLM 组合成完整 RAG 链路。', 1, 2, 'RAG,向量数据库,LangChain', 9, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY)),
(8, 'Linux 常用命令与 Shell 脚本入门', '日常运维必备的 Linux 命令与简单脚本编写。', '文件查找、日志分析、进程管理、定时任务与简单的 Shell 脚本示例，适合作为运维入门参考。', 1, 4, 'Linux,Shell,运维', 6, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 4 DAY), DATE_SUB(NOW(), INTERVAL 4 DAY)),
(9, 'MyBatis-Plus 条件构造器与分页插件', '熟练使用 Lambda 条件构造器与分页查询。', 'QueryWrapper、LambdaQueryWrapper、分页配置与逻辑删除，让 CRUD 开发更高效。', 1, 1, 'MyBatis-Plus,Java,后端', 11, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 5 DAY)),
(10, 'Redis 持久化：RDB 与 AOF 选型', '理解 RDB 与 AOF 的优缺点及生产环境配置建议。', '介绍两种持久化机制的原理、配置项以及如何根据业务场景选择或组合使用。', 1, 3, 'Redis,持久化,中间件', 4, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 6 DAY), DATE_SUB(NOW(), INTERVAL 6 DAY)),
(11, 'Prompt 工程：让大模型更听话', '常用提示词技巧与 few-shot 示例设计。', '系统提示、角色设定、思维链与少样本示例，提升与大模型对话的可控性与效果。', 1, 2, 'Prompt,大模型,AI', 8, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 7 DAY), DATE_SUB(NOW(), INTERVAL 7 DAY)),
(12, 'Nginx 反向代理与负载均衡配置', '使用 Nginx 做前后端分离与多实例负载均衡。', 'upstream、proxy_pass、静态资源与 HTTPS 配置，适合博客与小型项目部署。', 1, 4, 'Nginx,负载均衡,运维', 5, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 8 DAY), DATE_SUB(NOW(), INTERVAL 8 DAY)),
(13, 'Java 并发：synchronized 与 Lock', '理解线程安全与常见锁机制。', 'synchronized、ReentrantLock、volatile 与常见并发场景下的正确用法与注意事项。', 1, 1, 'Java,并发,多线程', 10, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 9 DAY), DATE_SUB(NOW(), INTERVAL 9 DAY)),
(14, 'Elasticsearch 全文检索入门', '使用 ES 实现博客搜索与高亮。', '索引 mapping、match 查询、高亮与简单聚合，为博客增加搜索能力。', 1, 3, 'Elasticsearch,搜索,中间件', 7, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 10 DAY)),
(15, 'LangChain Agent 与 Tools 实践', '用 Agent 调用工具完成多步任务。', 'Agent 执行流程、Tool 定义与注册，实现一个简单的文档问答 Agent 示例。', 1, 2, 'LangChain,Agent,AI', 6, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 11 DAY), DATE_SUB(NOW(), INTERVAL 11 DAY)),
(16, 'RabbitMQ 消息确认与死信队列', '保证消息可靠投递与异常处理。', 'Publisher Confirm、Consumer ACK、死信队列与延迟队列的常见用法。', 1, 3, 'RabbitMQ,消息队列,中间件', 9, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 12 DAY), DATE_SUB(NOW(), INTERVAL 12 DAY)),
(17, 'Docker 多阶段构建与镜像优化', '减小镜像体积并加快构建速度。', '多阶段构建、精简基础镜像与 .dockerignore 使用，让镜像更小更安全。', 1, 4, 'Docker,镜像,运维', 4, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 13 DAY), DATE_SUB(NOW(), INTERVAL 13 DAY)),
(18, 'Spring AOP 切面编程实战', '用 AOP 实现日志、权限与性能监控。', '@Aspect、@Around、切点表达式与在 Spring Boot 中统一处理横切逻辑。', 1, 1, 'Spring,AOP,Java', 8, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 14 DAY), DATE_SUB(NOW(), INTERVAL 14 DAY)),
(19, 'Vue 3 组合式 API 与响应式原理', 'Composition API 与 ref、reactive 使用要点。', '从 Options API 到 Composition API，理解响应式原理与组合式开发实践。', 1, 1, 'Vue,前端,组合式API', 12, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 15 DAY), DATE_SUB(NOW(), INTERVAL 15 DAY)),
(20, '向量数据库选型：Milvus 与 Chroma', '对比常见向量库在 RAG 场景下的适用性。', 'Milvus、Chroma、PGVector 等向量库的特点与在 LangChain 中的集成方式。', 1, 2, '向量数据库,RAG,AI', 5, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 16 DAY), DATE_SUB(NOW(), INTERVAL 16 DAY)),
(21, 'MySQL 主从复制与读写分离', '搭建主从复制并配合 Sharding-JDBC 做读写分离。', 'binlog、主从延迟与读写分离配置，提升读多写少场景下的性能与可用性。', 1, 3, 'MySQL,主从,高可用', 6, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 17 DAY), DATE_SUB(NOW(), INTERVAL 17 DAY)),
(22, '设计模式：工厂与策略模式', '常用设计模式在业务代码中的运用。', '简单工厂、工厂方法、抽象工厂与策略模式，减少 if-else 并提高可扩展性。', 1, 1, '设计模式,Java,架构', 7, 0, 'PUBLISHED', DATE_SUB(NOW(), INTERVAL 18 DAY), DATE_SUB(NOW(), INTERVAL 18 DAY));

INSERT IGNORE INTO post_skill_relation (post_id, skill_id) VALUES
(1, 2), (2, 4), (3, 3), (4, 6), (5, 4), (6, 2), (7, 3), (8, 6),
(9, 2), (10, 4), (13, 1), (14, 4), (15, 3), (16, 5), (18, 2), (19, 7), (20, 3), (22, 9);

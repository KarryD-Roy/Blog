<template>
  <div class="page auth-page">
    <div class="auth-card">
      ...
      <p class="auth-subtitle">加入 KARRY TECH BLOG</p>
      <form @submit.prevent="handleRegister" class="auth-form">
        <div class="form-group">
          <label class="form-label">用户名 *</label>
          <input v-model="form.username" type="text" class="input" placeholder="3-50个字符" required />
        </div>
        <div class="form-group">
          <label class="form-label">昵称</label>
          <input v-model="form.nickname" type="text" class="input" placeholder="你的显示名称" />
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input v-model="form.password" type="password" class="input" placeholder="至少6个字符" required />
        </div>
        <div class="form-group">
...
const handleRegister = async () => {
  ...
  if (form.username.length < 3) { errorMsg.value = '用户名至少3个字符'; return; }
  if (!form.password || form.password.length < 6) { errorMsg.value = '密码至少6个字符'; return; }
...
.auth-btn { margin-top: 0.5rem; width: 100%; padding: 1rem; font-size: 1rem; }
...
.auth-switch a { color: #ccff00; text-decoration: none; font-weight: bold; }
